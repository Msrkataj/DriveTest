    ...
    const monitorTaskStatus = async (taskId, parsedUserData) => {
        ...
        const checkStatus = async () => {
            ...
            try {
                ...
                if (status === 'completed') {
                    console.log('Task completed. Sending user data to the server...');
                    await sendUserDataToServer(parsedUserData); // Wyślij dane użytkownika do backendu
                    Alert.alert('Success', 'Logged in successfully!');
                    setLoading(false);
                    navigation.navigate('OfferSelection'); // Nawigacja do kolejnego ekranu
